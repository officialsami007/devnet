...
export function NewsletterForm() {
  ...
  return (
    <form className="newsletter-form" onSubmit={submit}>
      ...
      <div>
        <input id="newsletter-email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Work email" />
        <button type="submit" disabled={status === "loading"} aria-label="Subscribe">{status === "loading" ? "…" : "→"}</button>
      </div>
